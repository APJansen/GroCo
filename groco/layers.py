...
import groups
from utils import EquivariantPadding
...
class GroupConv2D(Conv2D):
    ...
    def __init__(self, group, kernel_size, allow_non_equivariance: bool=False, **kwargs):
        ...
        self.equivariant_padding = EquivariantPadding(allow_non_equivariance=allow_non_equivariance, kernel_size=kernel_size, **kwargs)
    ...
    def build(self, input_shape):
        """
        Check if the input is a signal on the group (rather than just on the grid) and stores that in attribute group_valued_input.
        If so, merge the group axis with the channel axis.
        Then run the parent class's build.
        Finally run the EquivariantPadding layer's build on the merged input.
        """
    ...
    def _group_transform_kernel(self):
        ...
        """
        transformed_kernel = self.group.action(self.kernel)
        if self.group_valued_input:
