...
class TestGroupPooling2D(TestCase):
    ...
    def test_pool_shape(self):
        for group in self.group_dict.values():
            ...
            self.assertEqual(pooled_signal.shape, self.pooled_shape[:-1] + (group.order, self.shape[-1]))
    ...
    def test_pool_shape_subgroup(self):
        for group in self.group_dict.values():
            for subgroup_name in group.subgroup.keys():
                ...
                pooled_signal = pool_layer(signal_on_group)
                self.assertEqual(pooled_signal.shape, self.pooled_shape[:-1] + (subgroup.order, self.shape[-1]))
        ...
        for group in self.group_dict.values():
            for subgroup_name in group.subgroup.keys():
                ...
                pool_layer = self.pool(
                    group=group, pool_size=2, strides=2, padding='same_equiv', subgroup=subgroup_name)
                equiv_diff = test_equivariance(
                    pool_layer, signal_on_group, group_axis=self.group_axis, subgroup=subgroup_name,
