from tensorflow.keras.layers import MaxPooling3D, AveragePooling3D, Layer
from groco.layers import GroupTransforms


class GroupPooling3D(Layer):
    """
    ...
    """
    def __init__(self, group, pool_type: str, allow_non_equivariance: bool = False, pool_size=(2, 2, 2), subgroup=None,
                 **kwargs):
...
class GroupMaxPooling3D(GroupPooling3D):
    """
...
class GroupAveragePooling3D(GroupPooling3D):
    """
    Layer for pooling on signals on a group. Allows for pooling on the grid as usual,
    but also on subgroups of the point group.
    Checks and restores equivariance in the same way as GroupConv2D.
