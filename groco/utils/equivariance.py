...
def check_equivariance(
    ...
    _check_shape(layer_signal, group_axis, group, target_group, "layer(signal)", f"target group {target_group}")
    ...
    g_layer_signal = group.action(
        layer_signal, spatial_axes=spatial_axes, group_axis=group_axis, new_group_axis=0,
        acting_group=acting_group, domain_group=target_group)
...
def _check_shape(signal, group_axis, group, subgroup, signal_string, group_string):
    if subgroup is not None:
        message = (signal_string + " not on " + group_string + ", " +
